...
export const LogLi =forwardRef((props:{log:Log,handleClick:Function},ref:React.Ref<HTMLDivElement>)=>{
    ...
    return(
        <div className={`flex flex-col sm:flex-row sm:justify-around align-middle text-center sm:items-center space-x-4 h-fit sm:h-16  border-2 p-4 m-2 rounded-md shadow-sm odd:bg-gray-50
        ...
        `} ref={ref} onClick={()=>{handleClick()}}>
            ...
            <NamedLabel description="Position" shortened={false} text={"x:"+log.localization_x.toString()+" y:"+log.localization_y.toString() } styleClasses=" "  clickHandler={moveToMap} widthClass="w-24"/>
            <div className="hidden sm:block w-0.5 h-full bg-gray-500"></div>
